...
- 🗄️ **Archive System** — Soft-delete tasks and workspaces with full restore capability
- 🖼️ **Profile Image Upload** — Upload, preview and remove your avatar photo
- ⚙️ **Settings** — Profile editing, password change, notification preferences, workspace overview & account deletion
...
| `PATCH` | `/api/auth/profile-picture` | 🖼️ Upload profile picture |
| `DELETE` | `/api/auth/profile-picture` | 🗑️ Remove profile picture |
...
| `PATCH` | `/api/tasks/:workspaceId/:taskId` | ✏️ Update a task |
...
The Settings page (`/settings`) includes 5 fully functional tabs:

| Tab | Features |
|-----|---------|
| 👤 **My Profile** | Update display name, upload/remove avatar photo |
| 🔒 **Security** | Change password with live strength indicator & eye-toggle |
| 🔔 **Notifications** | Toggle @mention email alerts on/off |
| 🏢 **My Workspaces** | View all workspaces with role badges, leave non-owned workspaces |
| ⚠️ **Danger Zone** | Permanently delete account with email confirmation |

---

## 🔒 Security & Notes

- 🔑 JWT tokens stored in `localStorage`, sent as `Bearer` tokens on every request
- 🌐 Google OAuth redirects to `/login-success?token=<jwt>` after authentication
- 📧 Mention emails only sent when recipient has `emailNotifications` enabled
- 🔢 Task IDs auto-increment starting from `124500`
- 🖼️ Profile pictures stored in `server/uploads/avatars/` — max 2MB, JPEG/PNG/WebP/GIF
